...
function PlayerView(uri, useViewOffset, returnView) {
    ...
    function showSubsMenu(timeout) {
      ...
      //  if (timeout) {
       //     clearTimeout(controlsTimer);
       //     controlsTimer = setTimeout(subsmenu, timeout);
       // }
    }    
	...
	this.render = function (container) {
        ...
        /*  lg is working with netcast type so no need for this  */
        if (media.mimeType) {
		...
		if (media.subtitles) {
             for (i = 0; i < media.subtitles.length; i++) {
                 var sub = media.subtitles[i];
                  ...
                  if(sub.selected && sub.key) {
                  video.subtitle = plexAPI.getURL(sub.key);
                  }
                  else if(sub.key)
                  video.subtitle = plexAPI.getURL(sub.key);
                ...
                // buildSubsMenu(sub);
                 ...
                 //console.log(media.subtitles[i].language)
                //  console.log('Loading subtitle ' + plexAPI.getURL(media.subtitles) + '...');
             }
            
           // video.subtitle= plexAPI.getURL(media.subtitles);
            //console.log('Loading subtitle ' + plexAPI.getURL(media.subtitles) + '...');
    ...
    window.NetCastPageLoadingIcon("enabled");
