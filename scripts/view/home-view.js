...
function HomeView() {
    ...
    var PRELOADED_IMAGES = 6;
    var PREVIEW_MENU_LOADING_DELAY = 0;
    ...
    function CreateBlobs(media) {
        ...
        var key = section.key;
            plexAPI.browse(plexAPI.recentlyAdded(key), function(container) {
                  ...
                  for (y = 0; y < container.media.length; y++) {
                      var thumb;
                      if(container.media[y].grandparentThumb)
                      thumb = container.media[y].grandparentThumb;
                      else
                      thumb = container.media[y].thumb;
                      
                      //console.log("http://192.168.0.187:32400"+thumb);
                      Blobs.imageBlob(thumb, "small");
                  }
            });
            plexAPI.browse(plexAPI.onDeck(key), function(container) {
                  var y = 0;
    ...
    this.onEnter = function() {
       ...
       // var parent = current.getAttribute('data-parent');
