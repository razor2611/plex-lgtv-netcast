...
function ListView(uri, returnView) {
 ...
 /*       
        if (Settings.useBlobs()) {
           if(Blobs.retrieveBlob(media.art, "art")) {
           backgroundLoader.load = Blobs.retrieveBlob(media.art, "art");
           console.log("conditie blob");
           }
           else
        ...
        */
        ...
        if (!media.type) {
            ...
            updateTopTitle(mediaContainer.title1);
        }
            ...
            updateTopTitle(mediaContainer.title1 + " : " + mediaContainer.title2);  
        }
    ...
    function buildScroller(mediaList) {
        ...
        for (var i = 0; i < n; i++) {
            ...
            var builder = [ "<div class='title-container'>" + media.title.encodeHTML() + "</div>"];
            ...
            scroller.appendChild(item);
             ...
             if(media.thumb) {
              if(!Blobs.retrieveBlob(media.thumb, "large")) {
                 Blobs.imageBlob(media.thumb, "large");
               }
              if(!Blobs.retrieveBlob(media.art, "art")) {
                 Blobs.imageBlob(media.art, "art");
               }               
    ...
    function updateTopTitle(text) {  
        document.getElementById('top-title').innerHTML = text;
    }
